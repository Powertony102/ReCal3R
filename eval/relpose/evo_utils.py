...
def matrix_to_tum_pose(c2w):
    xyz = c2w[:3, 3]
    qx, qy, qz, qw = Rotation.from_matrix(c2w[:3, :3]).as_quat()
    return np.concatenate([xyz, [qw, qx, qy, qz]], 0)


def load_seven_scenes_traj(gt_dir):
    pose_files = sorted(Path(gt_dir).glob("frame-*.pose.txt"))
    poses = [np.loadtxt(path).astype(np.float64) for path in pose_files]
    traj_tum = np.stack([matrix_to_tum_pose(pose) for pose in poses], 0)
...
def load_nrgbd_traj(gt_file):
    ...
    for i in range(0, len(lines), lines_per_matrix):
        ...
        has_nan = any("nan" in line for line in pose_lines)
        if len(pose_lines) < lines_per_matrix or has_nan:
            pose = np.eye(4, dtype=np.float64)
        else:
            pose = np.array(
                [[float(x) for x in line.split()] for line in pose_lines],
                dtype=np.float64,
            )
        pose[:, 1:3] *= -1.0
        poses.append(pose)
...
def load_scannet_traj(gt_path, frame_ids=None):
    gt_path = Path(gt_path)
    if gt_path.is_dir():
        if frame_ids is None:
            pose_files = []
            for path in gt_path.glob("*.txt"):
                if path.stem.isdigit():
                    pose_files.append(path)
            pose_files = sorted(pose_files, key=lambda path: int(path.stem))
        else:
            pose_files = [gt_path / f"{int(frame_id)}.txt" for frame_id in frame_ids]
        ...
        for pose_file in pose_files:
            if not pose_file.exists():
                continue
            try:
                pose = np.loadtxt(pose_file).astype(np.float64).reshape(4, 4)
            except Exception:
                continue
            if not np.isfinite(pose).all():
                continue
            poses.append(pose)
            timestamps.append(float(pose_file.stem))

        if len(poses) == 0:
            raise RuntimeError(f"No valid ScanNet pose files found in {gt_path}")

        traj_tum = np.stack([matrix_to_tum_pose(pose) for pose in poses], 0)
        timestamps_mat = np.array(timestamps, dtype=float)
        return traj_tum, timestamps_mat

    return load_replica_traj(str(gt_path))


def _load_tum_txt_trajectory(gt_traj_file):
    data = np.loadtxt(gt_traj_file, comments="#", ndmin=2)
    if data.shape[1] < 8:
        raise RuntimeError(f"Invalid TUM trajectory file: {gt_traj_file}")
    timestamps = data[:, 0].astype(float)
    xyz = data[:, 1:4].astype(float)
    quat_xyzw = data[:, 4:8].astype(float)
    quat_wxyz = quat_xyzw[:, [3, 0, 1, 2]]
    traj_tum = np.column_stack((xyz, quat_wxyz))
    return traj_tum, timestamps


def _match_tum_traj_to_timestamps(gt_traj_file, timestamps, max_difference=0.02):
    traj_tum, gt_timestamps = _load_tum_txt_trajectory(gt_traj_file)
    if timestamps is None:
        return traj_tum, gt_timestamps
    ...
    for timestamp in timestamps:
        while (
            gt_idx + 1 < len(gt_timestamps)
            and abs(gt_timestamps[gt_idx + 1] - timestamp)
            <= abs(gt_timestamps[gt_idx] - timestamp)
        ):
            gt_idx += 1
        if abs(gt_timestamps[gt_idx] - timestamp) <= max_difference:
            matched_traj.append(traj_tum[gt_idx])
            matched_timestamps.append(timestamp)

    if len(matched_traj) == 0:
        raise RuntimeError(
            f"No TUM-RGBD poses in {gt_traj_file} match the selected RGB timestamps"
        )
    if len(matched_traj) != len(timestamps):
        raise RuntimeError(
            f"Only matched {len(matched_traj)}/{len(timestamps)} TUM-RGBD poses in {gt_traj_file}"
        )
    return np.stack(matched_traj, 0), np.array(matched_timestamps, dtype=float)
...
def load_traj(
    ...
    if traj_format == "replica":
        ...
        if traj_format == "tum" and frame_ids is not None:
            traj_tum, timestamps_mat = _match_tum_traj_to_timestamps(
                gt_traj_file, frame_ids
            )
            skip = 0
            stride = 1
        else:
        ...
        traj_tum, timestamps_mat = load_seven_scenes_traj(gt_traj_file)
    elif traj_format == "nrgbd":
        traj_tum, timestamps_mat = load_nrgbd_traj(gt_traj_file)
    elif traj_format == "scannet":
        traj_tum, timestamps_mat = load_scannet_traj(gt_traj_file, frame_ids=frame_ids)
        if frame_ids is not None:
            skip = 0
            stride = 1
    else:
