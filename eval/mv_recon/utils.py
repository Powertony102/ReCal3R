...
def chamfer_distance(acc, comp, average=True):
    """Compute symmetric Chamfer distance from directed distances.

    Args:
        acc: Mean nearest-neighbor distance from predicted points to GT points.
        comp: Mean nearest-neighbor distance from GT points to predicted points.
        average: If True, return (acc + comp) / 2. Otherwise return acc + comp.
    """
    chamfer = acc + comp
    if average:
        chamfer *= 0.5
