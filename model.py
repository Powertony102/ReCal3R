...
from dust3r.mask_utils import (
    SAM2AutomaticRefiner,
    build_easi3r_dynamic_mask,
    compute_state_static_scores,
    upsample_patch_mask,
    view_img_to_uint8,
)
...
class ARCroco3DStereoConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        use_easi3r_mask=False,
        easi3r_mask_alpha=0.35,
        easi3r_mask_clusters=64,
        easi3r_mask_cluster_fuse_weight=0.5,
        easi3r_mask_use_otsu=True,
        easi3r_mask_reweight=1.0,
        easi3r_sam2_refine=False,
        easi3r_sam2_checkpoint="",
        easi3r_sam2_model_cfg="",
        **croco_kwargs,
    ):
        super().__init__()
        ...
        self.use_easi3r_mask = use_easi3r_mask
        self.easi3r_mask_alpha = easi3r_mask_alpha
        self.easi3r_mask_clusters = easi3r_mask_clusters
        self.easi3r_mask_cluster_fuse_weight = easi3r_mask_cluster_fuse_weight
        self.easi3r_mask_use_otsu = easi3r_mask_use_otsu
        self.easi3r_mask_reweight = easi3r_mask_reweight
        self.easi3r_sam2_refine = easi3r_sam2_refine
        self.easi3r_sam2_checkpoint = easi3r_sam2_checkpoint
        self.easi3r_sam2_model_cfg = easi3r_sam2_model_cfg
...
class ARCroco3DStereo(CroCoNet):
    ...
    def _compute_recal3r_update_mask(self, update_mask, raw_cross_attn_state):
        """
        Update mask: combines spatial visibility and
        measurement reliability to control state update strength.
        """
        # [L, B, H, N_state, N_obs]
        ...
        avg_attn_weights = attn_weights.mean(dim=(0, 2))  # [B, N_state, N_obs]
        n_obs = avg_attn_weights.shape[-1]
    ...
    def _build_sam2_refiner(self, device):
        if not getattr(self.config, "easi3r_sam2_refine", False):
            return None
        ckpt = getattr(self.config, "easi3r_sam2_checkpoint", "")
        model_cfg = getattr(self.config, "easi3r_sam2_model_cfg", "")
        if not ckpt or not model_cfg:
            printer.warning(
                "SAM2 refinement is enabled but checkpoint/config paths are missing. "
                "Skipping SAM2."
            )
            return None
        refiner = SAM2AutomaticRefiner(
            checkpoint=ckpt,
            model_cfg=model_cfg,
            device=device,
            logger=printer,
        )
        return refiner if refiner.is_available() else None
    ...
    def _forward_impl(self, views, ret_state=False):
        ...
        init_state_feat = state_feat.clone()
        init_mem = mem.clone()
        sam2_refiner = self._build_sam2_refiner(feat[0].device)
            ...
            if i  == 0:
                ...
                if self.config.model_update_type == "cut3r":
                    ...
                    with torch.no_grad():
                        ...
                        raw_recon_err = torch.norm(diff, p=2, dim=-1).mean().item()
    ...
    def forward_recurrent_lighter(self, views, device='cuda', ret_state=False):
        ...
        for i, _view in enumerate(views):
            ...
            if not sam2_initialized:
                sam2_refiner = self._build_sam2_refiner(device)
            ...
            if i  == 0 or reset_mask:
                ...
                if self.config.model_update_type == "cut3r":
                    ...
                    state_attn_tensor = rearrange(torch.cat(raw_cross_attn_state, dim=0), 'l h nstate nimg -> 1 nstate nimg (l h)') # [12, 16, 768, 1 + 576] -> [1, 768, 1 + 576, 12*16]
                    state_query_img_key = state_attn_tensor.mean(dim=(-1, -2))
                    update_mask1 = update_mask * torch.sigmoid(state_query_img_key)[..., None] * 1.0
                else:
                    raise ValueError(f"Invalid model type: {self.config.model_update_type}")
